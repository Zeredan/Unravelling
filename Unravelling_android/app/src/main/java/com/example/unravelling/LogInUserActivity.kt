                            ...
                            (Data.ServerSocket?.incoming?.receive() as Frame.Text?)?.readText()?.let{ registerResponse ->
                                when (registerResponse)
                                {
                                    "0" -> { bAlertDialogOpened.value = true }
                                        ...
                                        try {
                                            registerResponse.split(" ").run {
                                                File(
                                                    filesDir,
                                                    "UserData"
                                                ).also { it.createNewFile() }
                                                    .writeText("${this[0]} ${this[1]}")
                                            }
                                        }
                                        catch(e: Exception)
                                        {
            ...
            )
            {
                if (bAlertDialogOpened.value)
                {
                    ...
                    AlertDialog(
                        ...
                        title = {
                            Text("Такого пользователя не существует или пароль неверен", color = Color.Red)
                        }
                    )
                }
                Column(
                        ...
                        if ((userName.value.isNotEmpty()) && (password.value.isNotEmpty()))
                        {
                            Box(
                                modifier = Modifier.size(100.dp)
                                    .clickable {
                                        runBlocking {
                                            Data.ServerSocket?.send("logInUser ${userName.value} ${password.value}")
                                        }
                                    }
                            )
                            {
                                ...
                                Text("Войти", color = Color.Magenta)
                            }
                        }
                    }
